...
dotnet build
dotnet run
A API estará acessível em http://localhost:5204. (tem swagger)
5.2. Executando o Front-end (Nuxt 3)
Em outro terminal, navegue até o diretório do front-end e execute:
cd pinebank-web
npm run dev
O front-end estará acessível em http://localhost:3000.(demora uns 10segundos pra iniciar)
...
As dependências do projeto são gerenciadas separadamente para o back-end e o front-end.

1. Back-end (.NET API)
As dependências do back-end são definidas no arquivo PineBank.API.csproj. Para restaurá-las, execute:
cd src/PineBank.API
dotnet restore
...
Entendi, Guilherme. A seguir, apresento o README.md completo para o seu projeto PineBank Hackaton. Você pode copiar e colar este conteúdo diretamente no arquivo README.md do seu repositório no GitHub.

markdown
Copiar código
# PineBank Hackaton – Assistente Virtual Bancário Integrado

![PineBank Logo](https://github.com/pertodomato/PineBankHackaton/blob/main/assets/logo.png)
...
CURRENCY_API_BASE=https://economia.awesomeapi.com.br
IG_PAGE_ACCESS_TOKEN=SeuTokenDeAcessoInstagramAqui
⚠️ Atenção: Nunca comite o arquivo .env no repositório. Certifique-se de que ele está listado no .gitignore.
...
DotNetEnv
2. Front-end (Nuxt 3)
As dependências do front-end estão listadas no arquivo pinebank-web/package.json. Para instalá-las, execute:
cd pinebank-web
npm install
Principais Pacotes Utilizados:
...
Caso prefira utilizar Docker para gerenciar os serviços MongoDB e Redis, utilize o arquivo docker-compose.local.yml fornecido.
